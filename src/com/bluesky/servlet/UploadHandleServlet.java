		...
		try {
			...
			for (FileItem item : list) {
				...
				if (item.isFormField()) {
					...
					if (new InfoDao().getUploadByName(title)) {
						new InfoDao().insertInfo(title, "http://39.105.34.171:8080/img/"+saveFilename, info, type);
					} else {
						new InfoDao().changeInfo(new InfoDao().getid(title), title, "http://39.105.34.171:8080/img/"+saveFilename, info, type);
					}
